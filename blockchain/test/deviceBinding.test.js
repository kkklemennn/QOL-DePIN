...
describe("Device Binding", function () {
  ...
  describe("Initialization", function () {
    it("Should initialize the contract with Device Registry", async function () {
      ...
      expect(await deviceBinding.devicesRegistry()).to.equal(devicesRegistry.address);
    });
  });
  ...
  describe("Binding", function () {
    ...
    it("Should bind a device", async function () {
      ...
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(user.address);
      expect(await deviceBinding.getDevicesCount()).to.equal(1);
      expect(await deviceBinding.getOwnedDevices(user.address)).to.eql([DEVICE_ID_1]);
    });
    ...
    it("Should bind multiple devices", async function () {
      ...
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(user.address);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_2)).to.equal(user.address);
      expect(await deviceBinding.getDevicesCount()).to.equal(2);
      expect(await deviceBinding.getOwnedDevices(user.address)).to.eql([DEVICE_ID_1, DEVICE_ID_2]);
    });
    ...
    it("Should bind multiple devices to multiple users", async function () {
      ...
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(user.address);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_2)).to.equal(user_2.address);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_3)).to.equal(user.address);
      expect(await deviceBinding.getDevicesCount()).to.equal(3);
      expect(await deviceBinding.getOwnedDevices(user.address)).to.eql([DEVICE_ID_1, DEVICE_ID_3]);
      expect(await deviceBinding.getOwnedDevices(user_2.address)).to.eql([DEVICE_ID_2]);
    });
    ...
    it("Should not bind a device if it is already bound", async function () {
      await devicesRegistry.registerDevice(DEVICE_ID_1);
      await deviceBinding.bindDevice(DEVICE_ID_1, user.address);
      await expect(deviceBinding.bindDevice(DEVICE_ID_1, user.address)).to.be.revertedWith("device has already been bound");
    });

    it("Should not bind a device if it is not authorized", async function () {
      await expect(deviceBinding.bindDevice(DEVICE_ID_1, user.address)).to.be.revertedWith("device is not registered");
    });
    ...
    it("User can bind the device if isRegistered is true, isActive is false, and address is 0x00", async function () {
      await devicesRegistry.registerDevice(DEVICE_ID_1);
      //await devicesRegistry.suspendDevice(DEVICE_ID_1);  // Ensure the device is inactive
      await deviceBinding.bindDevice(DEVICE_ID_1, user.address);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(user.address);
      expect(await deviceBinding.getDevicesCount()).to.equal(1);
    });
    ...
    it("Admin (owner of smart contract) can bind the device whenever", async function () {
      await devicesRegistry.registerDevice(DEVICE_ID_1);
      await deviceBinding.connect(owner).bindDevice(DEVICE_ID_1, owner.address);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(owner.address);
      expect(await deviceBinding.getDevicesCount()).to.equal(1);
    });

    it("User cannot bind the device if it is active", async function () {
      await devicesRegistry.registerDevice(DEVICE_ID_1);
      await devicesRegistry.activateDevice(DEVICE_ID_1);  // Ensure the device is active
      await expect(deviceBinding.bindDevice(DEVICE_ID_1, user.address)).to.be.revertedWith("device is already bound or not available for binding");
    });
  });
  ...
  describe("Unbinding", function () {
    beforeEach(async function () {
      ...
      await devicesRegistry.activateDevice(DEVICE_ID_2);
    });
    ...
    it("Should unbind multiple devices", async function () {
      await deviceBinding.bindDevice(DEVICE_ID_2, user.address);
      await devicesRegistry.suspendDevice(DEVICE_ID_1);
      await devicesRegistry.suspendDevice(DEVICE_ID_2);
      await deviceBinding.unbindDevice(DEVICE_ID_1);
      await deviceBinding.unbindDevice(DEVICE_ID_2);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_1)).to.equal(ZERO_ADDR);
      expect(await deviceBinding.getDeviceOwner(DEVICE_ID_2)).to.equal(ZERO_ADDR);
      expect(await deviceBinding.getDevicesCount()).to.equal(0);
    });
    ...
    it("Should not unbind a device if it is not bound", async function () {
      await expect(deviceBinding.unbindDevice(DEVICE_ID_2)).to.be.revertedWith("device is not bound");
    });

    it("Should not unbind a device if it is not suspended", async function () {
      await expect(deviceBinding.unbindDevice(DEVICE_ID_1)).to.be.revertedWith("device must be suspended before unbinding");
    });

    it("Should not unbind a device if it is already unbound", async function () {
      await devicesRegistry.suspendDevice(DEVICE_ID_1);
      await deviceBinding.unbindDevice(DEVICE_ID_1);
      await expect(deviceBinding.unbindDevice(DEVICE_ID_1)).to.be.revertedWith("device is not bound");
    });

    it("Should not unbind a device if it is not owned by the sender", async function () {
      await devicesRegistry.suspendDevice(DEVICE_ID_1);
      await expect(deviceBinding.connect(badGuy).unbindDevice(DEVICE_ID_1)).to.be.revertedWith("not the device owner or admin");
    });
  });
});
